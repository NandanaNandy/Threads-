...
# def process_pdf_sequentially(pdf_path, total_pages, dpi=300, poppler_path="C:\\poppler\\bin"):
#     results = []
#     for page_number in range(1, total_pages + 1):
#         result = process_page(pdf_path, page_number, dpi, poppler_path)
...
#     results = process_pdf_sequentially(pdf_path, total_pages, dpi=150, poppler_path=poppler_path)
#     print("\n".join(results))
#     endtime = time.time()
#     print(f"\nTotal execution time (sequential): {endtime - starttime:.2f} seconds")
...
def process_pdf_multiprocess(pdf_path, total_pages, dpi=300, poppler_path="E:\Release-24.08.0-0\poppler-24.08.0\Library\bin"):
    args = [(pdf_path, page_number, dpi, poppler_path) for page_number in range(1, total_pages + 1)]
...
if __name__ == "__main__":
    ...
    total_pages = 4
    poppler_path = r"E:\Release-24.08.0-0\poppler-24.08.0\Library\bin"
    results = process_pdf_multiprocess(pdf_path, total_pages, dpi=150, poppler_path=poppler_path)
    print("\n".join(results))
    endtime = time.time()
